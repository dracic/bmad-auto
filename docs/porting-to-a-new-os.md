# Porting bmad-auto to a new OS

bmad-auto runs on Linux and macOS today, and on Windows **via WSL** (which _is_
...
| --- | -------------------- | ------------------------------------------------------- | ------------------------ |
| 1   | Terminal multiplexer | `TerminalMultiplexer` / `register_multiplexer`          | `BMAD_AUTO_MUX_BACKEND`  |
| 2   | Process lifecycle    | `ProcessHost` / `register_process_host`                 | `BMAD_AUTO_PROCESS_HOST` |
...
orthogonal **CLI axis** — teaching bmad-auto a new coding CLI — lives in the
...
**Contract:** `TerminalMultiplexer` (`src/automator/adapters/multiplexer.py`) — how
...
```python
from automator.adapters.multiplexer import register_multiplexer
...
- `name` — the key the `BMAD_AUTO_MUX_BACKEND` override selects by.
...
true, unless `BMAD_AUTO_MUX_BACKEND` forces one by name; tmux is the default
...
**Contract:** `ProcessHost` (`src/automator/process_host.py`) — the four pid
...
```python
from automator.process_host import register_process_host
...
`BMAD_AUTO_PROCESS_HOST` forces one by name; POSIX is the default fallback.
...
`_platform_preflight()` (`src/automator/cli.py`, called from `cmd_validate`) asks
the selected multiplexer for its `available()` / `version()` and names the selected
process host. A new OS therefore surfaces its readiness in `bmad-auto validate`
...
bundled helper script may `import automator` — so for pid lifecycle it should
...
> Out-of-tree plugin scripts distributed outside this repo can't assume `automator`
...
`tests/test_portability_guard.py` AST-scans `src/automator` and fails CI if a new
...
| -------------- | -------------------------------- | -------------------------------- |
| a multiplexer  | `BMAD_AUTO_MUX_BACKEND=psmux`    | `get_multiplexer.cache_clear()`  |
| a process host | `BMAD_AUTO_PROCESS_HOST=windows` | `get_process_host.cache_clear()` |
