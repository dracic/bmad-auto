...
- Per-story token budget (`max_tokens_per_story`, advisory, checked post-done) using the same cost-weighted total — cache reads counted at `cache_read_weight` (default 0.1, matching ~0.1x vendor billing); displayed totals stay raw.
