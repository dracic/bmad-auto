"""`bmad-auto diagnose`: a sanitized diagnostic dump of a run/sweep.
...
through the audited :mod:`automator.sanitize` chokepoint before rendering.

It mirrors :mod:`automator.probe`: typed findings → collectors → ``render_markdown``
...
def render_markdown(d: Diagnostics, *, pseudo: sanitize.Pseudonymizer | None = None) -> str:
    out: list[str] = []
    out.append("# bmad-auto diagnostic dump (sanitized)")
    out.append("")
    out.append(
    ...
    e = d.env
    out.append(_fmt_kv("bmad-auto version", e.package_version))
